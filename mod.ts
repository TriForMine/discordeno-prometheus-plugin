...
const handler = <B extends Bot = bot>(rawBot: B): Response => {
    ...
    let body = `# HELP memory_usage Memory usage from the main process
# TYPE memory_usage gauge
memory_usage{type="rss"} ${(Deno.memoryUsage().rss / 1000000).toFixed(2)}
memory_usage{type="heapTotal"} ${(Deno.memoryUsage().heapTotal / 1000000).toFixed(2)}
memory_usage{type="heapUsed"} ${(Deno.memoryUsage().heapUsed / 1000000).toFixed(2)}
memory_usage{type="external"} ${(Deno.memoryUsage().external / 1000000).toFixed(2)}${
